...
- `view-leave-settings` — Staff leave settings wizard (Journey Steps, 6 steps): 1) fiscal/work-year cutoff date, 2) approver list management, 3) annual leave quota per leave type (sick/vacation/personal, maternity, etc.), 4) leave conditions, 5) advance-notice day count, 6) notification settings.
...
- `view-job-license` — Job position & professional license info (position, license numbers, contract type).
