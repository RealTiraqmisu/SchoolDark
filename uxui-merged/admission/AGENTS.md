...
> [!IMPORTANT]
> โปรเจกต์นี้ถูกย้ายจากโฟลเดอร์เดี่ยว `uxui-noeysod/` เข้ามาอยู่เป็น **`uxui-merged/admission/`**
> ร่วมกับโปรโตไทป์อีกระบบ (`../schooldark/` — ระบบบุคลากร/การลา ที่ใช้ CSS เขียนเองและมีธีมมืด)
> เนื้อหา/โค้ดแต่ละหน้ายังเป็น Tailwind + ธีมขาวเดิมทั้งหมดตามกฎด้านบน — สิ่งที่รวมกันคือ **sidebar
> ซ้ายของทั้ง 2 ระบบ** เท่านั้น (ไม่รวม data layer, ไม่ unify design system ของเนื้อหา)
>
> **ทุกหน้าตอนนี้มี sidebar ซ้ายแบบเดียวกับ `../schooldark/` แทนแถบแท็บบน header เดิม** — head ของ
> ทุกไฟล์โหลด `<link rel="stylesheet" href="../shared/cross-nav.css">` และ body มี
> `<script src="../shared/cross-nav.js"></script>` เป็น**สิ่งแรกหลัง `<body>`** สคริปต์นั้นจะสร้าง
> `<aside class="sidebar" id="sidebar">` แบบ `position:fixed` ทางซ้ายให้เอง (ไม่มี `<aside>` อยู่ก่อน
> ในหน้า admission) แล้วดันเนื้อหาเดิมของหน้าด้วย `padding-left` บน `<body>` ผ่าน class
> `xnav-fixed-layout` ที่ใส่ให้บน `<html>` — แถบแท็บเดิมบน `<header>` (บอร์ดรับสมัคร/ข้อมูลนักเรียน/
> ตั้งค่าฟอร์ม) **ยังอยู่ใน markup เดิมทุกไฟล์ แค่ถูกซ่อนด้วย `data-legacy-tabs`** (กฎ CSS จุดเดียวใน
> `cross-nav.css`) ยังไม่ได้ลบทิ้ง เผื่อย้อนกลับมาใช้ในอนาคต
>
> **เพิ่มหน้า `.html` ใหม่ในโฟลเดอร์นี้ = ต้องไปเพิ่ม item ใน `NAV_GROUPS` ของ `../shared/cross-nav.js`
> ด้วยเสมอ** (แหล่งข้อมูลเมนูจุดเดียวของทั้งโปรเจกต์ — คุม sidebar ของทั้ง 2 ระบบ) ไม่งั้นหน้าใหม่จะ
> เข้าถึงจากเมนูไม่ได้ — ดู `../CLAUDE.md` หัวข้อ "จุดเชื่อมของเมนูรวมในแต่ละหน้า" ก่อนแก้
>
> ⚠️ modal/dropdown/แถบลอย (bulk-action bar, toast) ที่ใช้ `position: fixed` เดิมของหน้านี้ยังทำงาน
> ปกติ แต่บางตัวที่จัดกึ่งกลางจอแนวนอน (เช่น bulk-action bar ใน `students.html`/`index.html` ที่ใช้
> `left-1/2 -translate-x-1/2`) จะกึ่งกลาง**ทั้งวิวพอร์ต** ไม่ใช่กึ่งกลางพื้นที่เนื้อหาที่เหลือหลังหัก
> sidebar ออก — เป็นข้อจำกัดที่รู้อยู่แล้ว (คอสเมติกเล็กน้อย) ยังไม่ได้แก้

```text
uxui-merged/admission/         # (เดิมคือโฟลเดอร์ uxui-noeysod/)
...
└── tasks/
    ...
    ── อยู่นอกโฟลเดอร์นี้ (ระดับ uxui-merged/) ──
    ../CLAUDE.md                # กฎของโฟลเดอร์รวม อ่านก่อนเริ่มงานข้ามระบบ
