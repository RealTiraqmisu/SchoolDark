...
> โดยรวมกัน**เฉพาะระดับเมนู** — ระบบนี้ยังคง Tailwind + ธีมขาว + โครงหน้าเดิมไว้ทั้งหมดตามกฎด้านบน
>
> สิ่งที่เพิ่มเข้ามาในทุกหน้าคือ **เมนูข้ามระบบ**: `<span id="cross-nav-topbar"></span>` ในแถบแท็บของ
> `<header>` และ `<script src="../shared/cross-nav.js"></script>` ก่อน `</body>` — สคริปต์นั้นจะเรนเดอร์
> ปุ่ม dropdown "ระบบอื่น" ด้วยคลาส Tailwind เดิมของหน้า แล้วเรียก `lucide.createIcons()` ให้เอง
>
> **เพิ่มหน้า `.html` ใหม่ในโฟลเดอร์นี้ = ต้องไปเพิ่มรายการใน `../shared/cross-nav.js` ด้วยเสมอ**
> (แหล่งข้อมูลเมนูจุดเดียวของทั้งโปรเจกต์) ไม่งั้นหน้าใหม่จะเข้าถึงจากเมนูไม่ได้ — ดู `../CLAUDE.md`
